								...
								if ($i == 0) :			
									...
									if (!empty($video_link)) :
										if (strpos($video_link, 'vimeo.com') !== false) {
											...
											$iframe = '<iframe src="https://player.vimeo.com/video/'.$video_id.'?title=0&byline=0&portrait=0" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe>';
										} else {
											parse_str( parse_url( $video_link, PHP_URL_QUERY ), $my_array_of_vars );
											$video_id = $my_array_of_vars['v'];		
											$thumbnail = "http://img.youtube.com/vi/".$video_id."/maxresdefault.jpg";	
											$title = $this->get_youtube_title($video_id);
										 	$iframe = '<iframe src="https://www.youtube.com/embed/'.$video_id.'?&autoplay=1" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
										}
										echo $iframe;			
								...
								foreach ( $settings['pafe_video_playlist'] as $item) :
									...
									if (!empty($video_link)) :
										if (strpos($video_link, 'vimeo.com') !== false) {
											...
											$hash = unserialize(file_get_contents("http://vimeo.com/api/v2/video/$video_id.php"));
											$thumbnail= $hash[0]['thumbnail_medium'];  
											$iframe = '<iframe src="https://player.vimeo.com/video/'.$video_id.'?autoplay=1?title=0&byline=0&portrait=0" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe>';
										} else {
											parse_str( parse_url( $video_link, PHP_URL_QUERY ), $my_array_of_vars );
											$video_id = $my_array_of_vars['v'];		
											$thumbnail = "http://img.youtube.com/vi/".$video_id."/maxresdefault.jpg";	
